...
# 设置工作目录，请根据实际情况修改
setwd("E:/FILE/Masterexp/Paper/1_mother_choice/1_data/picture/MC/")
...
growth_L <- read.csv("growth_rate.csv")
growth_L$groups = paste(growth_L$P,growth_L$F, sep="")

shapiro.test(growth_L$rate)
leveneTest(rate~P*F,data=growth_L) 
gr_L.aov <- aov(rate~P+F+P:F,data = growth_L) 
summary(gr_L.aov)
tukey <- TukeyHSD(gr_L.aov)


dt <- growth_L %>% group_by(P,F) %>%
...
cld <- TukeyHSD(gr_L.aov) %>% multcompLetters4(gr_L.aov,.)
le <- as.data.frame.list(cld$`P:F`)
dt$tukey <- le$Letters
dt$groups = paste(dt$P,dt$F, sep="")
dt

merge_df <- merge(dt[,c("w","tukey","groups")],growth_L[,c("rate","groups")],by="groups",all=F)
...
p_growthrate
ggsave("growthrate_L.svg", width =180, height= 90 , units ="mm",dpi=1000)


###############################ir


ir_L <- read.csv("ir_MC.csv")
ir_L$groups = paste(ir_L$P,ir_L$F, sep="")


shapiro.test(ir_L$rate) 
leveneTest(rate~P*F,data=ir_L) 
ir_L.aov <- aov(rate~P+F+P:F,data =ir_L) 
summary(ir_L.aov)
TukeyHSD(ir_L.aov, which = "P:F")

dt <- ir_L %>% group_by(P,F) %>%
  summarise(w=mean(rate), sd = sd(rate),
            n = n(),
            se = sd / sqrt(n) ) %>%
  arrange(desc(w)) %>%
...
cld <- TukeyHSD(ir_L.aov) %>% multcompLetters4(ir_L.aov,.)
le <- as.data.frame.list(cld$`P:F`)
dt$tukey <- le$Letters
dt$groups = paste(dt$P,dt$F, sep="")
dt
group_order <- c( "LC","CC", "CL", "LL")
dt$groups <- factor(dt$groups, levels = group_order)

merge_df <- merge(dt[,c("w","tukey","groups")],ir_L[,c("rate","groups")],by="groups",all=F)
# 按照"CC, CM, MC, MM"的顺序设置group的因子水平
group_order <- c( "LC","CC", "CL", "LL")
merge_df$groups <- factor(merge_df$groups, levels = group_order)

p_ir <- ggplot(dt,aes(x=groups,y=w,fill=groups)) +
  geom_bar(width=0.5,stat="identity",colour="black")+
  geom_errorbar(aes(ymin=w-se,
                    ymax=w+se),width=0.2)+
  geom_text(data=dt,aes(x=groups,y=w+0.05,label=tukey),color="black",size=5)+
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 16),
        ...
        plot.margin=unit(rep(1,4),'mm'))+
  scale_fill_manual(values =c("#B2E2DC", "#33A02C", "#E8CF92", "#D6A228"))+
  scale_y_continuous(breaks=seq(0,0.5,0.1),expand = c(0, 0),limits = c(0,0.51))+
  labs(x="Groups",y="Intrinsic growth rate")



p_ir
ggsave("intrinsic_rate_L.svg", width =180, height= 90 , units ="mm",dpi=1000)

####################################offspring
off_L <- read.csv("offspring_L.csv")
off_L$groups = paste(off_L$P,off_L$F, sep="")


shapiro.test(off_L$offspring) 
leveneTest(offspring~P*F,data=off_L) 
off_L.aov <- aov(offspring~P+F+P:F,data =off_L) 
summary(off_L.aov)
TukeyHSD(off_L.aov, which = "P:F")


dt <- off_L %>% group_by(P,F) %>%
  summarise(w=mean(offspring), sd = sd(offspring),
            n = n(),
            se = sd / sqrt(n)) %>%
  arrange(desc(w)) %>%
  ungroup()

cld <- TukeyHSD(off_L.aov) %>% multcompLetters4(off_L.aov,.)
le <- as.data.frame.list(cld$`P:F`)
dt$tukey <- le$Letters
dt$groups = paste(dt$P,dt$F, sep="")
dt
...
merge_df <- merge(dt[,c("w","tukey","groups")],off_L[,c("offspring","groups")],by="groups",all=F)
# 按照"CC, CM, MC, MM"的顺序设置group的因子水平
group_order <- c( "LC","CC", "CL", "LL")
merge_df$groups <- factor(merge_df$groups, levels = group_order)


p_off <- ggplot(merge_df, aes(x=groups, y=offspring,fill=groups))+ 
  geom_violin(trim = F,width=0.8,size=0.8)+ 
  geom_boxplot(width=0.15,fill="white",size=0.8, outlier.colour= NA )+ 
  geom_text(data=merge_df,aes(x=groups,y=w+4,label=tukey),color="black",size=5)+
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 16),
        ...
        plot.margin=unit(rep(1,4),'mm'))+
  scale_fill_manual(values =c("#B2E2DC", "#33A02C", "#E8CF92", "#D6A228"))+
  scale_y_continuous(breaks=seq(0,9,1),expand = c(0, 0),limits = c(-0.5,9.5))+
  ylab("Total offspring per female (ind.)") + xlab("Groups") 


p_off
ggsave("off_L.svg", width =180, height= 90 , units ="mm",dpi=1000)

####################R0
R0_L <- read.csv("R0_L.csv")
R0_L$groups = paste(R0_L$P,R0_L$F, sep="")


shapiro.test(R0_L$rate) 
leveneTest(rate~P*F,data=R0_L) 
R0_L.aov <- aov(rate~P+F+P:F,data =R0_L) 
summary(R0_L.aov)
TukeyHSD(R0_L.aov, which = "P:F")

dt <- R0_L %>% group_by(P,F) %>%
  summarise(w=mean(rate), sd = sd(rate),
            n = n(),
            se = sd / sqrt(n)) %>%
  arrange(desc(w)) %>%
...
cld <- TukeyHSD(R0_L.aov) %>% multcompLetters4(R0_L.aov,.)
le <- as.data.frame.list(cld$`P:F`)
dt$tukey <- le$Letters
dt$groups = paste(dt$P,dt$F, sep="")
dt
...
merge_df <- merge(dt[,c("w","tukey","groups")],R0_L[,c("rate","groups")],by="groups",all=F)
merge_df$groups <- factor(merge_df$groups, levels = group_order)

p_R0 <- ggplot(dt,aes(x=groups,y=w,fill=groups)) +
  geom_bar(width=0.5,stat="identity",colour="black")+
  geom_errorbar(aes(ymin=w-se,
                    ymax=w+se),width=0.2)+
  geom_text(data=dt,aes(x=groups,y=w+1,label=tukey),color="black",size=5)+
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 16),
  ...
  labs(x="Groups",y=expression(paste("Net reproduction rate ( ",R[0],")")))



p_R0
ggsave("R0_L.svg", width =180, height= 90 , units ="mm",dpi=1000)

#################################G0
G0_L <- read.csv("G0_L.csv")
G0_L$groups = paste(G0_L$P,G0_L$F, sep="")


shapiro.test(G0_L$rate) 
leveneTest(rate~P*F,data=G0_L) 
G0_L.aov <- aov(rate~P+F+P:F,data =G0_L) 
summary(G0_L.aov)
TukeyHSD(G0_L.aov, which = "P:F")

dt <- G0_L %>% group_by(P,F) %>%
  summarise(w=mean(rate), sd = sd(rate),
            n = n(),
            se = sd / sqrt(n)) %>%
  arrange(desc(w)) %>%
...
cld <- TukeyHSD(G0_L.aov) %>% multcompLetters4(G0_L.aov,.)
le <- as.data.frame.list(cld$`P:F`)
dt$tukey <- le$Letters
dt$groups = paste(dt$P,dt$F, sep="")
dt
...
merge_df <- merge(dt[,c("w","tukey","groups")],G0_L[,c("rate","groups")],by="groups",all=F)


p_G0 <- ggplot(dt,aes(x=groups,y=w,fill=groups)) +
  geom_bar(width=0.5,stat="identity",colour="black")+
  geom_errorbar(aes(ymin=w-se,
                    ymax=w+se),width=0.2)+
  geom_text(data=dt,aes(x=groups,y=w+1.1,label=tukey),color="black",size=5)+
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 16),
        ...
        plot.margin=unit(rep(1,4),'mm'))+
  scale_fill_manual(values =c("#B2E2DC", "#33A02C", "#E8CF92", "#D6A228"))+
  scale_y_continuous(breaks=seq(0,6,1),expand = c(0, 0),limits = c(0,6.5))+
  labs(x="Groups",y=expression(paste("Total reproduction rate ( ",G[0],")")))



p_G0
ggsave("G0_L.svg", width =180, height= 90 , units ="mm",dpi=1000)
...
combined_plot <- plot_grid(
  p_density,p_growthrate,  p_survival,p_reproductive,p_ir,p_off,p_R0,p_G0,
...
# 保存组合图形
ggsave('MC_all.svg', combined_plot, width = 360, height = 420, units = "mm", dpi = 1000)
ggsave('MC_all.tiff', combined_plot, width = 360, height = 420, units = "mm", dpi = 1000)
